...
def refresh_game(game_id, home_team_id, away_team_id, home_name, away_name):
    ...
    There used to be a Phase 3 that re-called API-Football so own_goals /
    cards / player_minutes picked up post-match revisions. It went with
    API-Football itself: own goals, cards and substitutions now arrive as
    ordinary events in Phase 2, so a revision lands with the re-download
    rather than needing a second source to be asked separately.
